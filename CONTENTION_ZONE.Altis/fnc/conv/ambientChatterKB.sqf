...
_handle = _this spawn {
	...
	if (!(alive _receiver) OR ((!isNil "IP_AmbientChatter_Running") && {IP_AmbientChatter_Running})) exitWith {};
	...
	while {alive _receiver} do {
		...
		if ((count _NPCs > 0) && {(isNil "IP_SimpleSentence_Talking") OR {!IP_SimpleSentence_Talking}}) then {
			...
			if ((count _sentences > 0) && {!(_speaker getVariable ["IP_SimpleSentence_Talking", false])}) then {
