...
_bikb = [_this, 4, "\Campaigns\IP_CMP_ICE\cfg\sentences.bikb", [""]] call BIS_fnc_param;
_count = count _speakers;

if (typeName _sentences == "STRING") then {
	_sentences = [_sentences] call IP_fnc_getSentencesFromConversationKB;
};
...
for "_i" from 0 to ((count _sentences) - 1) do {
	...
	if (_sentence != "") then {[_speaker, _receiver, _topic, _sentence, _whichChat, _bikb] call IP_fnc_simpleSentenceKB};
