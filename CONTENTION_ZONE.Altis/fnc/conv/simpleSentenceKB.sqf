...
#define TELL if (count _kbTellParams == 0) then {[[_speaker, [_receiver, _topic, _sentence]], "IP_fnc_kbTell", false] call BIS_fnc_MP} else {[[_speaker, [_receiver, _topic, _sentence, _kbTellParams, _forceRadio]], "IP_fnc_kbTell", false] call BIS_fnc_MP};
#define WAIT waitUntil {_speaker getVariable ["IP_SimpleSentence_Talking", false]};
#define TELLLOCAL if (count _kbTellParams == 0) then {_speaker kbTell [_receiver, _topic, _sentence]} else {_speaker kbTell [_receiver, _topic, _sentence, _kbTellParams, _forceRadio]};
#define WAITLOCAL waitUntil {_speaker kbWasSaid [_receiver, _topic, _sentence, 60]};
...
if !(alive _speaker) exitWith {false};
IP_SimpleSentence_Talking = true;
_speaker setVariable ["IP_SimpleSentence_Talking", true, true];
...
_bikb = [_this, 5, "\Campaigns\IP_CMP_ICE\cfg\sentences.bikb", [""]] call BIS_fnc_param;
_subs = if (!(isNil "IP_ShowHud") && {!IP_ShowHud}) then {false} else {[_this, 6, true, [false]] call BIS_fnc_param};
...
if ((local _speaker) && {!(_speaker kbHasTopic _topic)}) then {
	...
	[[_speaker, _topic, _bikb], "IP_fnc_kbAddTopic", false] call BIS_fnc_MP;
};
...
if (_subs) then {
	if (count(toArray _text) > 200) then {42 cutRsc ["IP_DLG_SIMPLESENTENCE_LARGE", "PLAIN"]} else {42 cutRsc ["IP_DLG_SIMPLESENTENCE", "PLAIN"]}; 
	((uiNamespace getVariable "DLG_SimpleSentence") displayCtrl 1100) ctrlSetStructuredText (parseText _text);

	if (_liveFeed && {(vehicle _speaker) == _speaker} && {!visibleMap}) then {
		...
		_cfg = missionConfigFile >> "PPEffects";		
		if (!(isNil "IP_PPEffect") && {isText(_cfg >> IP_PPEffect >> "colorCorrections")}) then {
			_params = call(compile(getText(_cfg >> IP_PPEffect >> "colorCorrections")));
			"rendertarget0" setPiPEffect ([3, 1] + _params);
		};
		
		if (!(isNil "IP_PPEffect") && {isText(_cfg >> IP_PPEffect >> "filmGrain")}) then {
			_params = call(compile(getText(_cfg >> IP_PPEffect >> "filmGrain")));
			"camrender0" setPiPEffect ([6, 1] + _params);
		};
		
		if (local _speaker) then {
			TELLLOCAL
			WAITLOCAL
			_speaker setVariable ["IP_SimpleSentence_Talking", true, false];
		} else {
			TELL
			WAIT
		};
		...
		_picture = _speaker getVariable ["IP_Avatar", "images\defaultAvatar.jpg"];
