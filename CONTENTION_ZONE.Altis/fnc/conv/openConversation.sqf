...
.jpg - "images\defaultAvatar.jpg"
*/

private ["_getArguments", "_first", "_second", "_conversation", "_config", "_handle", "_picture", "_rank", "_faction", "_str", "_sentences", "_i", "_arguments", "_text", "_key", "_responses", "_exit", "_sound", "_music", "_expression"];
_getArguments = {
...
_rank = if ((side _first) != civilian) then {if (!isNil "IP_fnc_getRank") then {(([_first, true] call IP_fnc_getRank) + "<br/>")} else {(rank _first) + "<br/>"}} else {""};
_faction = if ((_first getVariable ["IP_Faction", ""]) != "") then {((getText(missionConfigFile >> "Factions" >> (_first getVariable "IP_Faction") >> "displayNameShort")) + "<br/>")} else {""};
...
_rank = if ((side _second) != civilian) then {if (!isNil "IP_fnc_getRank") then {(([_second, true] call IP_fnc_getRank) + "<br/>")} else {(rank _second) + "<br/>"}} else {""};
_faction = if ((_second getVariable ["IP_Faction", ""]) != "") then {((getText(missionConfigFile >> "Factions" >> (_second getVariable "IP_Faction") >> "displayNameShort")) + "<br/>")} else {""};
...
_key = if (isText(_config >> "key")) then {(getText(_config >> "key"))} else {_conversation};
