_this spawn {	
	...
	_reward = (getNumber(missionConfigFile >> 'ShopItems' >> 'Misc' >> 'Medikit' >> 'price')) * 0.6;
	...
	if (!CHECK) then {
		(leader _ownerGrp) removeItem "Medikit";
	...
	_ownerGrp setVariable ["IP_MDocNeed_Started", nil, true];
};